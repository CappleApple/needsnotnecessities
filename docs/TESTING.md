...
- With the default `thirst.drink_below_stage_percentage = 90`, verify drinks work in each of the lowest four default Thirst stages but not in the best stage. Confirm non-drink use items remain unaffected.
...
- Define two different food-group definitions that each grant `+1 armor`, use one ingredient from each in a recipe, and verify both the Shift tooltip and active meal show a combined `+2 armor` modifier.
- Use two ingredients matched by the same vegetable definition and verify a `+1 armor` definition totals `+1.5 armor` with the default `meal.same_group_diminishing_factor = 0.5`. Add a third and verify `+1.75 armor`; test `1.0`, `0.25`, and `0.0` factors as well. Confirm each recipe tag slot is counted once regardless of how many item alternatives it contains.
...
- Override a state with every datapack notification type, combinations, and no `notifications` array. Verify outputs occur only on entering that state; bundled defaults should only play the three lowest-state placeholder sounds.
